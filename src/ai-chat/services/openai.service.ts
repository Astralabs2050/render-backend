...
export class OpenAIService {
  ...
  async classifyIsFashionPrompt(prompt: string): Promise<boolean> {
    try {
      const response = await this.axiosInstance.post(this.apiUrl, {
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'Classify if the USER text is a fashion design request (garment, apparel, outfit, fabric, style, construction, tailoring, fashion accessories). Reply with a single token: yes or no. No explanations.' },
  ...
  // FIXED: uses PromptService.getSystemPrompt() instead of hardcoded string
  // This was the duplicate personality bug — Astra had two different voices
  ...
  async generateDesignMetadata(prompt: string, imageUrl?: string): Promise<any> {
    try {
      const messages: any[] = [
        ...
        { role: 'user', content: `Extract metadata for this fashion design: "${prompt}"${imageUrl ? ' (Reference image provided)' : ''}` }
      ];
      const response = await this.axiosInstance.post(this.apiUrl, {
        model: 'gpt-4o',
        messages,
        ...
        functions: [{
          ...
          parameters: {
            ...
            properties: {
              ...
              category: { type: 'string', description: 'Category of the design (e.g., Dress, Suit, Streetwear)' },
              price: { type: 'number', description: 'Suggested price in USD' },
              timeframe: { type: 'number', description: 'Estimated days to make' },
              colors: { type: 'array', items: { type: 'string' }, description: 'Color tags' },
              description: { type: 'string', description: 'Brief description of the design' }
            },
      ...
      return { name: 'Custom Design', category: 'Dress', price: 100, timeframe: 7, colors: ['white'], description: prompt };
    } catch (error) {
  ...
  async extractDesignInfo(content: string): Promise<any> {
    try {
      ...
      catch { return { garmentType: null, style: null, colors: null, size: null, occasion: null }; }
    } catch (error) {
  ...
  // NEW: Analyses a fabric photo using GPT-4o Vision
  // Called by interactive-chat.service.ts when user uploads a fabric photo
  // Returns a description like "deep emerald silk with natural sheen and fluid drape"
  // This description is then injected into the Gemini image generation prompt
  async describeFabricFromPhoto(imageBase64: string): Promise<string> {
    try {
      const response = await this.axiosInstance.post(this.apiUrl, {
        model: 'gpt-4o',
        messages: [
          {
            role: 'system',
            content: 'You are a fashion fabric expert. Describe the fabric in the image in 1-2 sentences focusing on: colour, texture, weight, and sheen. Be specific and visual. Example: "A deep emerald green silk with natural sheen and fluid drape, medium weight."',
          },
          {
            role: 'user',
            content: [
      ...
      this.logger.error(`Fabric description error: ${error.message}`);
      return 'your fabric';
    }
  }

  // ── Gemini image generation — completely unchanged ────────────────────────
        ...
        for (const part of response.parts) {
          if (part.inlineData && part.inlineData.data) {
            ...
            const permanentImageUrl = await this.cloudinaryService.uploadImage(imageBuffer, {
              ...
              transformation: { width: 1024, height: 1024, crop: 'fit', quality: 'auto', format: 'auto' }
            });
  ...
  private async storeImagePermanently(temporaryUrl: string, prompt: string): Promise<string> {
    try {
      const imageResponse = await axios.get(temporaryUrl, { responseType: 'arraybuffer', timeout: 30000 });
  ...
  private async enhanceDesignPrompt(prompt: string, referenceImageBase64?: string): Promise<string> {
    try {
      const messages: any[] = [
        { role: 'system', content: 'You are an expert fashion design prompt engineer. Transform user descriptions into detailed, professional Gemini image generation prompts for fashion design. Focus on technical fashion details, fabric textures, silhouettes, and professional fashion illustration style.' },
        { role: 'user', content: `Transform this fashion design request into a detailed Gemini image generation prompt: "${prompt}"` }
      ];
      if (referenceImageBase64) {
        messages[1].content = { type: 'text', text: `Transform this fashion design request into a detailed Gemini image generation prompt, considering the reference image provided: "${prompt}"` };
        messages.push({ role: 'user', content: [{ type: 'image_url', image_url: { url: `data:image/jpeg;base64,${referenceImageBase64}` } }] });
      }
      const response = await this.axiosInstance.post(this.apiUrl, { model: 'gpt-4o', messages, temperature: 0.7, max_tokens: 300 });
      return response.data.choices[0].message.content;
    } catch (error) {
  ...
  private async enhanceConsistentDesignPrompt(prompt: string, baseStylePrompt?: string, variationIndex: number = 0, referenceImageBase64?: string): Promise<string> {
    try {
      ...
      const messages: any[] = [
        { role: 'system', content: `You are an expert fashion design prompt engineer. Create consistent image generation prompts that maintain the same artistic style across variations. ${consistencyInstructions}CRITICAL: All three variations MUST be uniform in style and hyper-realistic. Focus on: same lighting style, same illustration technique, same color palette approach, same level of detail, same background style, photorealistic rendering, professional fashion photography quality.` },
        { role: 'user', content: `Create variation ${variationIndex + 1} of this fashion design: "${prompt}". ${variationInstructions[variationIndex] || variationInstructions[0]}. MUST maintain consistent artistic style and hyper-realistic appearance across all variations.` }
      ];
      if (referenceImageBase64) {
        messages[1].content += ' Consider the reference image provided for fabric/texture details.';
        messages.push({ role: 'user', content: [{ type: 'image_url', image_url: { url: `data:image/jpeg;base64,${referenceImageBase64}` } }] });
      }
      const response = await this.axiosInstance.post(this.apiUrl, { model: 'gpt-4o', messages, temperature: 0.3, max_tokens: 300 });
      return response.data.choices[0].message.content;
    } catch (error) {
      this.logger.error(`Consistent prompt enhancement error: ${error.message}`);
      const fallbackStyles = ['front view, hyper-realistic professional fashion photography style', '3/4 angle view, hyper-realistic professional fashion photography style', 'detailed close-up view, hyper-realistic professional fashion photography style'];
  ...
  async generateDesignImageWithDALLE(prompt: string): Promise<string> {
    try {
      ...
      const response = await this.axiosInstance.post('https://api.openai.com/v1/images/generations', {
        model: 'dall-e-3', prompt: enhancedPrompt, n: 1, size: '1024x1024', quality: 'standard', response_format: 'url'
      });
      const temporaryUrl = response.data.data[0].url;
      this.logger.log(`DALL-E image generated: ${temporaryUrl}`);
      return await this.storeImagePermanently(temporaryUrl, prompt);
    } catch (error) {
  ...
  private async enhanceDesignPromptForDALLE(prompt: string): Promise<string> {
    try {
      const response = await this.axiosInstance.post(this.apiUrl, {
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: 'You are an expert fashion design prompt engineer. Transform user descriptions into detailed, professional DALL-E 3 image generation prompts for fashion design. Focus on technical fashion details, fabric textures, silhouettes, and professional fashion illustration style. Keep prompts under 1000 characters.' },
          { role: 'user', content: `Transform this fashion design request into a detailed DALL-E 3 image generation prompt: "${prompt}"` }
        ],
  ...
  private async enhanceConsistentDesignPromptForDALLE(prompt: string, baseStylePrompt?: string, variationIndex: number = 0): Promise<string> {
    try {
      const consistencyInstructions = baseStylePrompt ? `Maintain the same art style, lighting, and visual aesthetic as: "${baseStylePrompt}". ` : 'Establish a consistent art style. ';
      const variationInstructions = ['front view with white background', '3/4 angle view with white background', 'detailed close-up of key design features with white background'];
      const response = await this.axiosInstance.post(this.apiUrl, {
        model: 'gpt-4o',
        messages: [
          { role: 'system', content: `You are an expert fashion design prompt engineer. Create consistent prompts that maintain the same artistic style across variations. ${consistencyInstructions}CRITICAL: All three variations MUST be uniform in style and hyper-realistic. Keep under 1000 characters.` },
          { role: 'user', content: `Create variation ${variationIndex + 1} of this fashion design: "${prompt}". ${variationInstructions[variationIndex] || variationInstructions[0]}. MUST maintain consistent artistic style and hyper-realistic appearance across all variations.` }
        ],
      ...
      const fallbackStyles = ['front view, hyper-realistic professional fashion photography', '3/4 angle view, hyper-realistic professional fashion photography', 'detailed close-up view, hyper-realistic professional fashion photography'];
